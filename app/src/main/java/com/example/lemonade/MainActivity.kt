...
class MainActivity : AppCompatActivity() {
    ...
    private fun setViewElements() {
        ...
        var resultLemonText = ""
        var resultLemonImage = 0
        // TODO: set up a conditional that tracks the lemonadeState
        when (lemonadeState) {
            SELECT -> {
                resultLemonText = getString(R.string.lemon_select)
                resultLemonImage = R.drawable.lemon_tree
            }
            SQUEEZE -> {
                resultLemonText = getString(R.string.lemon_squeeze)
                resultLemonImage = R.drawable.lemon_squeeze
            }
            DRINK -> {
                resultLemonText = getString(R.string.lemon_drink)
                resultLemonImage = R.drawable.lemon_drink
            }
            RESTART -> {
                resultLemonText = getString(R.string.lemon_empty_glass)
                resultLemonImage = R.drawable.lemon_restart
            }
        }

        // TODO: for each state, the textAction TextView should be set to the corresponding string from
        //  the string resources file. The strings are named to match the state
        textAction.text = resultLemonText

        // TODO: Additionally, for each state, the lemonImage should be set to the corresponding
        //  drawable from the drawable resources. The drawables have the same names as the strings
        //  but remember that they are drawables, not strings.
        lemonImage?.setImageResource(resultLemonImage)
